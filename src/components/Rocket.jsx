...
const Rocket = () => {
	...
	const updateRotation = (event) => {
		const posX =
			event.clientX -
			(rocketRef.current.getBoundingClientRect().left + rocketRef.current.width / 2);
		const posY =
			event.clientY -
			(rocketRef.current.getBoundingClientRect().top + rocketRef.current.height / 2);
	...
	useEffect(() => {
		...
		const animateRocket = () => {
			setPositionDrag((prevPositionDrag) => ({
				left: prevPositionDrag.left + (position.left - prevPositionDrag.left) / 15,
